...
def color_match(c1, c2, tolerance=15):
...
class DragCaptureWindow(tk.Toplevel):
    def __init__(self, parent, callback):
        ...
        self.geometry(f"{total_width}x{total_height}+{min_x}+{min_y}")
...
def show_positions_on_screen():
    ...
    top = tk.Toplevel(app)
    top.geometry(f"{total_width}x{total_height}+{min_x}+{min_y}")
